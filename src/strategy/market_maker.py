...
class MarketMaker:
    ...
    def build_quote(self, *, best_bid: float | None, best_ask: float | None, inventory: float) -> Quote | None:
        ...
        skew_bps = skew_ratio * (self.config.spread_bps / 2.0)
        bid = max(0.01, mid - spread / 2.0 - mid * (skew_bps / 10_000.0))
        ask = min(0.99, mid + spread / 2.0 - mid * (skew_bps / 10_000.0))
        bid = round(bid, 2)
        ask = round(max(ask, bid + 0.01), 2)
        return Quote(bid=bid, ask=ask, mid=round(mid, 4), spread=round(ask - bid, 4), size=self.config.order_size, skew_bps=round(skew_bps, 2))
